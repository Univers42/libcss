...
import { CATEGORY_META, registry, Breadcrumb } from '@libcss/studio';

interface VariantGalleryViewProps {
  componentId: string;
...
function generatePresetsFromDimensions(entry: ComponentEntry): VariantPreset[] {
  ...
  // Cartesian product
  function cartesian(
    dimensions: readonly import('@libcss/studio').VariantDimensionDef[],
    idx: number,
...
export function VariantGalleryView({
  ...
  const allPresets = useMemo<VariantPreset[]>(() => {
    ...
    const auto = entry.variantDimensions
      ...
      : [];
    // Manual presets take priority; auto-generated fill in
