...
export function Bookmark({
  ...
  const classes = cn(
    'prisma-bookmark',
    !thumbnail && 'prisma-bookmark--no-thumbnail',
    className,
  );
  ...
  return (
    <a className={classes} href={url} target="_blank" rel="noopener noreferrer">
      <div className="prisma-bookmark__content">
        ...
        {description && (
          <span className="prisma-bookmark__description">{description}</span>
        )}
        <span className="prisma-bookmark__url">
          {favicon && (
            <img className="prisma-bookmark__favicon" src={favicon} alt="" />
          )}
