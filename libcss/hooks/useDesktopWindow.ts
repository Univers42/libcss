...
export function useDesktopWindow(): DesktopWindowAPI {
  ...
  const maximize  = useCallback(() => { api?.maximize(); }, [api]);
  const close     = useCallback(() => { api?.close(); }, [api]);
  const zoomIn    = useCallback(() => { api?.zoomIn(); }, [api]);
  const zoomOut   = useCallback(() => { api?.zoomOut(); }, [api]);
  const zoomReset = useCallback(() => { api?.zoomReset(); }, [api]);
