...
/* ── Shared props type for inner components ─────────── */
...
function SplitBranch({
  ...
  paneAnimation,
  className = '',
}: SharedPaneProps & { readonly node: Extract<SplitNode, { type: 'split' }>; readonly className?: string }) {
...
function SplitLeaf({
  ...
  paneAnimation,
  className = '',
}: SharedPaneProps & { readonly node: Extract<SplitNode, { type: 'leaf' }>; readonly className?: string }) {
  const { group } = node;
  ...
  return (
    ...
    >
      <TerminalTabs
        ...
        onDragStart={(gId, tId) => { _dragSource = { groupId: gId, tabId: tId }; }}
        ...
        onDropEdge={(targetGroupId, direction) => {
          if (_dragSource) {
            split.moveTabToNewSplit(_dragSource.groupId, _dragSource.tabId, targetGroupId, direction);
            _dragSource = null;
          }
        }}
      />
