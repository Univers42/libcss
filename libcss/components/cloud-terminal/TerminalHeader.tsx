  ...
  return (
    <header className={cn('ct-header', className)}>
      ...
      <div className="ct-header__left">
        {(onClose || onMinimize || onMaximize) && (
          <div className="ct-header__traffic">
            {onClose && <button className="ct-header__traffic-dot ct-header__traffic-dot--close" onClick={onClose} title="Close" aria-label="Close window" type="button" />}
            {onMinimize && <button className="ct-header__traffic-dot ct-header__traffic-dot--minimize" onClick={onMinimize} title="Minimize" aria-label="Minimize window" type="button" />}
            {onMaximize && <button className="ct-header__traffic-dot ct-header__traffic-dot--maximize" onClick={onMaximize} title="Maximize" aria-label="Maximize window" type="button" />}
          </div>
        )}
      ...
      <div className="ct-header__actions">
        {extra}
        {onClear && (
          <button className="ct-header__btn" onClick={onClear} title="Purge Buffer" aria-label="Clear terminal">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>
          </button>
        )}
        {onToggleFullscreen && (
          <button className="ct-header__btn" onClick={onToggleFullscreen} title={isFullscreen ? 'Exit Fullscreen' : 'Fullscreen'} aria-label="Toggle fullscreen">
            {isFullscreen ? (
